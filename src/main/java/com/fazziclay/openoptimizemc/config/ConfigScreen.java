...
public class ConfigScreen extends Screen {
    ...
    private void addButton(int x, int y, int w, int h, String text, BooleanSupplier o, BooleanSupplier o1, String... tooltip) {
        ButtonWidget button = new ButtonWidget(x, y, w, h, Text.empty(), localButton -> stateButton(localButton, text, o1.getAsBoolean()), (button1, matrices, mouseX, mouseY) -> {
            if (tooltip.length > 0)renderTooltip(matrices, Text.of(tooltip[0]), mouseX, mouseY);
        });
    ...
    protected void init() {
        ...
        addButton(10, height - 30, 200, 15, "Advanced Profiler [Shift + F3]", config::isAdvancedProfiler, config::toggleAdvancedProfiler, "Advanced call push() and pop() for more performance information (Debug Pie in Shift+F3 menu)");
        ...
        update = new ButtonWidget(10, height - 60, 200, 20, Text.of("§c(!)§a Update Available! (click to open URL)"), button -> {
